@extends('supplier::layouts.sightseeing')
@push('style')
<style>
    .error-from{
        color: red;
...
@push('scripts')
    <script>
        ...
        $(document).on("change", "#addSurge", function(){
            if($(this).val()){
                $('#addDiscount').removeClass('validate');
            }else{
                $('#addDiscount').addClass('validate');
            }
            });
        $(document).on("change", "#addDiscount", function(){
            if($(this).val()){
                console.log($(this).val());
                $('#addSurge').removeClass('validate');
            }else{
                $('#addSurge').addClass('validate');
            }
            });
        ...
        function getCitiesByCountry(id) {
            ...
            //$('#from_city_id').html(`<option><i class="fa fa-refresh fa-spin"></i></option`);
            $.ajax({
                type: 'get',
                url: '{{ url('getcities') }}',
                data: {
                ...
                beforeSend:function(){
                    $('#to_city_id').html('<option>Loading...  </option>');
                    $('#from_city_id').html('<option>Loading...  </option>');
                    $('#cities_between').html('<option>Loading...  </option>');
