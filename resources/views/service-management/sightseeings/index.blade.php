...
@endpush
@php
$routeFor = \Route::currentRouteName() == 'sightseeings.index' ? 'undrafted' : 'drafted';
@endphp
...
@endif
    @if (auth()->user()->hasAddPermission($routeName))
        <a class="btn btn-info pull-right mx-2" href="{{ route('sightseeings.create') }}"><i class="fa fa-plus"
                aria-hidden="true"></i> Create New
            Sightseeing</a>
    @endif
    @if ($routeFor == 'undrafted')
        <a type="button" class="btn btn-danger text-light mx-2" href="{{ route('sightseeings.drafted') }}"><i
...
@push('scripts')
    ...
    <script>
        function delColumn(id) {
            ...
            var ajaxType = 'delete';
            var url = "{!! url('services/sightseeings') !!}" + "/" + id;
            var data = {
                '_token': token,
                'id': id
            };
